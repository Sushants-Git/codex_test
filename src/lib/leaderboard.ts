...
import { CHALLENGE_END, CHALLENGE_START, THIRTY_MINUTES_MS, challengeWindowMillis } from "./challenge";
...
export async function fetchLeaderboard(limit = 100): Promise<LeaderboardRow[]> {
  ...
  const rows = participantDocs
    .map((doc) => {
      ...
      const lastSyncedAt = stepsDoc?.lastSyncedAt ?? stepsDoc?.updatedAt ?? null;
...
export function queueParticipantSync(ids: Array<ObjectId | string>) {
  const participantIds = ids.map((id) => (typeof id === "string" ? new ObjectId(id) : id));
  ...
  setTimeout(() => {
    refreshParticipants(idsToSync)
      ...
      .finally(() => {
        idsToSync.forEach((id) => pendingSyncs.delete(id.toHexString()));
      });
  }, 0);
}
...
async function refreshParticipants(participantIds: ObjectId[]) {
  const client = await getMongoClient();
  const db = client.db();
  const participantsCollection = db.collection<ParticipantDocument>(COLLECTION_PARTICIPANTS);
  ...
  for (const participant of participants) {
    await stepsCollection.updateOne(
      ...
      {
        $setOnInsert: { createdAt: new Date(), participantId: participant._id },
  ...
  await Promise.allSettled(participants.map((participant) => refreshParticipant(participant, db)));
}
...
async function refreshParticipant(participant: ParticipantDocument, db: Db) {
  if (!participant.googleTokens?.refreshToken) {
    await markSyncError(participant._id, "Missing Google refresh token; reconnect account.", db);
    return;
  }
  ...
  try {
    const {
      accessToken,
      updatedTokens,
    } = await ensureAccessToken(participant.googleTokens);
    ...
    await Promise.all([
      stepsCollection.updateOne(
        { participantId: participant._id },
        {
          ...
          $setOnInsert: { createdAt: now, participantId: participant._id },
        },
        { upsert: true },
      ),
    ...
    console.error(`Failed to refresh participant ${participant._id.toString()}`, error);
    await markSyncError(
      participant._id,
      error instanceof Error ? error.message : "Unknown error during Google Fit sync.",
      db,
    );
  }
}

async function markSyncError(participantId: ObjectId, message: string, db?: Db) {
  let database: Db;
  if (db) {
    database = db;
  } else {
